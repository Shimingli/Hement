...
import com.shiming.imageloader.listener.IGetBitmapListener;
import com.shiming.imageloader.listener.IGetDrawableListener;
import com.shiming.imageloader.listener.IImageLoaderListener;
import com.shiming.imageloader.listener.ImageSize;
import com.shiming.imageloader.okhttp.OnGlideImageViewListener;
import com.shiming.imageloader.okhttp.OnProgressListener;
import com.shiming.imageloader.okhttp.ProgressManager;
import com.shiming.imageloader.tranform.BlurBitmapTranformation;
import com.shiming.imageloader.tranform.GlideCircleTransformation;
import com.shiming.imageloader.tranform.RoundBitmapTranformation;
...
public class GlideImageLoaderClient implements IImageLoaderClient {
    ...
    public void displayImageInResource(Fragment fragment, int resId, ImageView imageView) {
        GlideApp.with(fragment).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).into(imageView);
    }
    ...
    public void displayImageInResourceTransform(Activity fragment, int resId, ImageView imageView, Transformation transformation, int errorResId) {
        GlideApp.with(fragment).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).apply(requestOptionsTransform(errorResId,errorResId,transformation)).into(imageView);
    }
    ...
    public void displayImageInResourceTransform(Context context, int resId, ImageView imageView, Transformation transformation, int errorResId) {
        GlideApp.with(context).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).apply(requestOptionsTransform(errorResId,errorResId,transformation)).into(imageView);
    }
...
//                .into(imageView);
    ...
    public void displayImageInResource(Fragment fragment, int resId, ImageView imageView, BitmapTransformation transformations) {
        GlideApp.with(fragment).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).transform(transformations).into(imageView);
    }
    ...
    public void displayImageInResource(Context context, int resId,  ImageView imageView, int defRes) {
        GlideApp.with(context).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).placeholder(defRes).error(defRes).into(imageView);
    }

    @Override
    public void displayImageInResource(Fragment fragment, int resId, ImageView imageView, int defRes) {
        GlideApp.with(fragment).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).placeholder(defRes).error(defRes).into(imageView);
    }
    //关心context
    @Override
    public void displayImageInResource(Context context, int resId,  ImageView imageView, int defRes, BitmapTransformation transformations) {
        GlideApp.with(context).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).placeholder(defRes).error(defRes).transform(transformations).into(imageView);
    }
    //关心fragment
    @Override
    public void displayImageInResource(Fragment fragment, int resId, ImageView imageView, int defRes, BitmapTransformation transformations) {
        GlideApp.with(fragment).load(resId).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).placeholder(defRes).error(defRes).transform(transformations).into(imageView);
    }
}
